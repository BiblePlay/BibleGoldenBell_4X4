...
const LOCAL_STORAGE_SOFT_LIMIT = 2_500_000

function normalizeQuestion(
  question: Omit<QuizQuestion, 'score'> & Partial<Pick<QuizQuestion, 'score'>>,
): QuizQuestion {
  ...
  return {
    ...
    score: 0,
    choices:
      question.answerType === 'multiple' && Array.isArray(question.choices)
...
function isValidQuestionSet(value: unknown): value is QuizQuestion[] {
  return Array.isArray(value) && value.length === EXPECTED_QUESTION_COUNT
}

function normalizeSet(value: QuizQuestion[]): QuizQuestion[] {
  return [...value]
...
function openQuestionDb(): Promise<IDBDatabase> {
...
async function readQuestionsFromDb(): Promise<QuizQuestion[] | null> {
  try {
...
async function writeQuestionsToDb(questions: QuizQuestion[]): Promise<void> {
  const database = await openQuestionDb()
...
function loadLocalQuestions(): QuizQuestion[] | null {
  for (const key of [STORAGE_KEY, LEGACY_STORAGE_KEY]) {
    try {
      const saved = window.localStorage.getItem(key)
      if (!saved) continue
      const parsed = JSON.parse(saved)
      if (isValidQuestionSet(parsed)) return normalizeSet(parsed)
    } catch {
      // Try the next source.
    }
  }
...
function cacheSmallQuestionSet(questions: QuizQuestion[]): void {
  try {
...
export async function loadQuestionsFromProject(): Promise<QuizQuestion[]> {
  ...
  try {
    ...
    if (!isValidQuestionSet(parsed)) throw new Error('invalid seed')
...
export async function uploadProjectAsset(
  questionId: string,
  kind: 'questionImage' | 'answerImage' | 'video' | 'audio',
  file: Blob,
  _filename: string,
): Promise<string> {
